...
class Model(object):
    def __init__(self, num_k=7, emb_dim=200, vocab_size=10000, use_clip=True, learning_rate=0.01, category_size=4):
        ...
        # self.x = tf.placeholder(dtype=tf.int32, shape=(None, num_k, emb_dim))
        self.x = tf.placeholder(dtype=tf.float32, shape=(None, num_k, emb_dim))
    ...
    def build_model(self):
        x_emb_4d = tf.expand_dims(self.x, -1) # expand dimension 1 for 3-d
        # 20 * 200 * 1
        ...
        # 14 * 1 * 256
        self.text_vec = tf.reshape(conv_l3, [-1, conv_l3.shape[1]*256])
