...
class Model(object):
    def __init__(self, num_k=7, emb_dim=128, vocab_size=10000, use_clip=True, learning_rate=0.01, category_size=4):
    ...
    def build_model(self):
        ...
        # 7 * 128 * 1
        conv_f1 = self.get_var(name='conv_f1', shape=[2, self.emb_dim, 1, 256])
        ...
        # 5 * 1 * 256 / 6*1
        conv_f2 = self.get_var(name='conv_f2', shape=[2, 1, 256, 256])
        ...
        # 3 * 1 * 256 / 5*1
        conv_f3 = self.get_var(name='conv_f3', shape=[2, 1, 256, 256])
        ...
        # 1 * 1 * 256 / 4*1
        self.text_vec = tf.reshape(conv_l3, [-1, 4*256])
        dense_1 = Dense(128, dtype=tf.float32, name='dense_1')
        layer_1 = tf.nn.tanh(dense_1(self.text_vec))
        print(self.text_vec.shape)
        print(layer_1.shape)
        self.out_layer = Dense(self.category_size, dtype=tf.float32, name='out_layer')
        self.word_prob = tf.nn.softmax(self.out_layer(layer_1))
        print(self.word_prob.shape)
        self.out_y = tf.argmax(self.word_prob, 1)

    def build_loss(self):
        print(self.y.shape)
        self.cross_entropy = -tf.reduce_sum( tf.one_hot(tf.to_int32(tf.reshape(self.y, [-1])), self.category_size, 1.0, 0.0) * tf.log(tf.clip_by_value(tf.reshape(self.word_prob, [-1, self.category_size]), 1e-20, 1.0)), 1)
